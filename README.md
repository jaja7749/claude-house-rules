...
|---|---|---|
| **safe-agent** | PreToolUse guard (blocks privilege escalation, persistence, exfiltration, git/gh injection; confirms push, `rm -rf`, new dependencies and outbound data first), plus machine rules loaded into every session, plus `/security-setup` and `/github-access` | enabled |
...
| **permissions.deny** | Read, Edit, Grep and Glob against `~/.ssh`, `.env*`, `*.pem`, `.git/`, shell rc files | manual step |
...
`git status`, `diff`, `add`, `commit` and `log` are local and work inside the sandbox. Only commands
needing the network or credentials fail by default. Enabling them with `--github` makes the guard their
only gatekeeper:

| | |
|---|---|
| **Runs** | `gh pr list/view/diff/checks/status`, `gh run list/view/watch`, `gh workflow list/view`, `gh issue list/view`, read-only `gh api`, `gh auth status` |
| **Confirms first** | `gh pr create/merge/comment`, `gh issue create`, `gh workflow run`, `gh run rerun/cancel`, `gh secret set`, `gh release create`, `gh api -X POST`, `git push` |
...
Prerequisites: `brew install gh` and `gh auth login`. Logging in is yours to run; the rules forbid Claude
from completing login or OAuth flows.
